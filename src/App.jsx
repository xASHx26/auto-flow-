...
function generatePlaywrightPythonSync(testCase, variables, settings = {}, trackLines = false) {
...
`;
  ...
  const lineMap = {};
  let currentLineNumber = code.split('\n').length;
  ...
  (testCase.actions || []).forEach((a, i) => {
    ...
    if (a.command === "open") {
      ...
      block = `${cmt}  # capture mode: ${a.value || "always"}\n${assertCode}    page.screenshot(path=${ss})\n\n`;
    ...
    code += block;
    const linesAppended = block.split('\n').length - 1;
    lineMap[i] = { start: currentLineNumber, end: currentLineNumber + linesAppended - 1 };
    currentLineNumber += linesAppended;
  });

  code +=
`${hasTrace ? "    context.tracing.stop(path=os.path.join(SCREENSHOTS_DIR, \"trace.zip\"))\n" : ""}    context.close()\n    browser.close()\n\n\nwith sync_playwright() as pw:\n    run(pw)\n${hasVerify ? "\nprint_summary()\n" : ""}`;
  return trackLines ? { code, lineMap } : code;
}

function generatePlaywrightPythonAsync(testCase, variables, settings = {}, trackLines = false) {
  const varLines = Object.entries(variables || {})
    .map(([k, v]) => `    "${esc(k)}": "${esc(v)}",`).join("\n");
  const hasSS = (testCase.actions || []).some(a => a.command === "captureScreenshot");
  const base  = (testCase.name || "test").replace(/\s+/g, "_");
...
function generatePytestPlaywright(testCase, variables, settings = {}, trackLines = false) {
  const varLines = Object.entries(variables || {})
    .map(([k, v]) => `    "${esc(k)}": "${esc(v)}",`).join("\n");
  const base = (testCase.name || "test").replace(/\s+/g, "_");
...
#
# Usage:
#   pytest test_${base.toLowerCase()}.py -v
# ==============================================================
...
`;
  ...
  (testCase.actions || []).forEach((a, i) => {
    ...
    if (a.command === "open") {
      ...
      block = `${cmt}\n    page.screenshot(path=f"step_{n}_screenshot.png")\n\n`;
    } else if (a.command === "verifyText") {
      block = `${cmt}\n${sc}    expect(${loc}).to_contain_text("${val}")\n\n`;
    } else if (a.command === "verifyElementPresent") {
      block = `${cmt}\n${sc}    expect(${loc}).to_be_visible()\n\n`;
    } else if (a.command === "selectWindow") {
      block = `${cmt}\n    page = page.context.pages[-1]  # switch to newest tab\n\n`;
    } else if (a.command === "refresh") {
      block = `${cmt}\n    page.reload()\n\n`;
    } else {
      block = `${cmt}\n${sc}    # TODO: handle "${a.command}"\n\n`;
  ...
  return trackLines ? { code: out_code, lineMap } : out_code;
}

function generatePlaywrightJS(testCase, variables, settings = {}, trackLines = false) {
  const varLines = Object.entries(variables || {})
    .map(([k, v]) => `  "${esc(k)}": "${esc(v)}",`).join("\n");
...
function generateSeleniumPython(testCase, variables, settings = {}, trackLines = false) {
  const varLines = Object.entries(variables || {})
    .map(([k, v]) => `    "${esc(k)}": "${esc(v)}",`).join("\n");
  const _hasSS = (testCase.actions || []).some(a => a.command === "captureScreenshot");
  const base  = (testCase.name || "test").replace(/\s+/g, "_");
...
try:\n`;
  ...
  const lineMap = {};
  let currentLineNumber = py.split('\n').length;
...
`except Exception as _e:\n    print(f"Test failed: {_e}")\n    driver.save_screenshot(os.path.join(SCREENSHOTS_DIR, "error_screenshot.png"))\n    raise\n\nfinally:\n    print("Test completed.")\n    time.sleep(2)\n    driver.quit()\n`;
  return trackLines ? { code: py, lineMap } : py;
}
...
function ExportModal({ testCase, variables, settings, onClose }) {
  ...
  const getCode = () => {
    if (activeTab === "pytest") return generatePytestPlaywright(testCase, variables, settings);
  ...
  <div className="h-10 bg-[#2b2b2b] border-b border-gray-700 flex items-center px-3 gap-2 justify-between">
    ...
    <div className="flex items-center gap-1 relative">
      ...
      <button
        ...
        className={`p-1 px-2 mx-1 rounded transition-all flex items-center gap-1.5 text-[10px] font-bold border ${showCodePreview ? "bg-blue-500/20 text-blue-400 border-blue-500/40 shadow-[0_0_8px_rgba(59,130,246,0.3)]" : "text-gray-400 border-transparent hover:bg-gray-700 hover:text-white"}`}
...
function Sidebar({ testCases, selectedIndex, onSelect, onSave, onLoad }) {
  return (
...
export default function App() {
  ...
  const [previewLang, setPreviewLang] = useState("pytest");
  ...
  const { code: previewCode, lineMap } = useMemo(() => {
    const tc = testCases[selectedTestCase] || {};
    if (previewLang === "pytest") {
      return generatePytestPlaywright(tc, variables, settings, true);
    } else if (previewLang === "pw_py_sync") {
      return generatePlaywrightPythonSync(tc, variables, settings, true);
    } else if (previewLang === "pw_py_async") {
      return generatePlaywrightPythonAsync(tc, variables, settings, true);
    } else if (previewLang === "pw_js") {
      return generatePlaywrightJS(tc, variables, settings, true);
    } else if (previewLang === "selenium_py") {
      return generateSeleniumPython(tc, variables, settings, true);
    }
    return { code: "", lineMap: {} };
  }, [testCases, selectedTestCase, variables, settings, previewLang]);
            ...
            <div style={{ width: codePreviewResize.size, minWidth: 200, flexShrink: 0 }} className="flex flex-col bg-[#1e1e1e] overflow-hidden min-h-0 relative">
              <div className="flex items-center justify-between px-3 py-1.5 bg-[#252525] border-b border-gray-700 shrink-0">
                ...
                <select
                  ...
                  className="text-[9px] font-bold text-emerald-400 bg-emerald-900/40 border border-emerald-700/50 px-1.5 py-0.5 rounded font-mono outline-none cursor-pointer appearance-none text-center"
                >
                  <option value="pytest">pytest</option>
                  <option value="pw_py_sync">playwright (sync)</option>
                  <option value="pw_py_async">playwright (async)</option>
                  <option value="pw_js">playwright (js)</option>
                  <option value="selenium_py">selenium (py)</option>
                </select>
              </div>
              <div
                ref={codePreviewRef}
              ...
              >
                {previewCode.split("\n").map((rawLine, i) => {
                  ...
                  // Extract comments
                  const comments = [];
                  const commentRegex = isJs ? /(\/\/[^\n]*)/g : /(#[^\n]*)/g;
                  coloured = coloured.replace(commentRegex, (m) => {
                    comments.push(m);
                    return `__COM${comments.length - 1}__`;
                  });

                  // Format keywords and numbers
                  const pyKeywords = /\b(def|import|from|return|if|else|elif|for|in|with|as|not|and|or|True|False|None|await|async|pass|raise|try|except|finally)\b/g;
                  const jsKeywords = /\b(const|let|var|await|async|function|return|if|else|for|in|of|try|catch|true|false|null|undefined|require)\b/g;
                  coloured = coloured
                    .replace(isJs ? jsKeywords : pyKeywords, '<span style="color:#569CD6">$1</span>')
                    .replace(/\b(page|context|browser|expect|pytest|chromium)\b/g, '<span style="color:#4EC9B0">$1</span>')
                    .replace(/\b(\d+)\b/g, '<span style="color:#B5CEA8">$1</span>');

                  // Restore comments and strings with formatting
                  coloured = coloured.replace(/__COM(\d+)__/g, (_, idx) => `<span style="color:#6A9955">${comments[idx]}</span>`);
                  coloured = coloured.replace(/__STR(\d+)__/g, (_, idx) => `<span style="color:#CE9178">${strings[idx]}</span>`);
                  return (
                    <div
                      key={i}
                      data-line={lineNum}
